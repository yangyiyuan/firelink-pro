...
const SignalInstanceModule = (function() {
    ...
    function getGroupLabel(value) {
        return SceneDataModule.getGroupLabel(value);
    }
    ...
    function renderInstanceList() {
        ...
        list.innerHTML = filtered.map(inst => {
            ...
            return `
                <button type="button" class="auto-template-item w-full text-left rounded-xl border px-3 py-3 transition-colors ${active ? 'border-jd-primary bg-jd-primaryLight/40' : 'border-jd-cardBorder bg-white hover:border-jd-primaryBorder'}" onclick="SignalInstanceModule.selectInstance('${escapeAttr(inst.id)}')">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button class="btn-secondary px-2.5 py-1 rounded-md text-xs" onclick="SignalInstanceModule.duplicateInstance('${escapeAttr(inst.id)}')">复制</button>
                        <button class="btn-secondary px-2.5 py-1 rounded-md text-xs text-red-500 hover:text-red-600" onclick="SignalInstanceModule.confirmDelete('${escapeAttr(inst.id)}')">删除</button>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="space-y-2">
                    <div>
                        ...
                        <input type="text" class="jd-input w-full px-3 py-2 rounded-lg text-sm bg-white" id="instNameInput" value="${escapeAttr(d.name || '')}" placeholder="例如：1号楼传输装置-单点报警" maxlength="80">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" class="jd-input w-full px-3 py-2 rounded-lg text-sm bg-white" id="instDescInput" value="${escapeAttr(d.description || '')}" placeholder="可选描述" maxlength="300">
                    </div>
                    <div>
                        <label class="block text-xs text-jd-textSecondary mb-1">标签（逗号分隔）</label>
                        <input type="text" class="jd-input w-full px-3 py-2 rounded-lg text-sm bg-white" id="instTagsInput" value="${escapeAttr((d.tags || []).join(','))}" placeholder="例如：A厂,1号楼" maxlength="100">
                    </div>
                </div>
                <div class="border-t border-jd-cardBorder pt-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <input type="text" class="jd-input w-full px-3 py-2 rounded-lg text-sm bg-white font-mono" id="instSourceAddrInput" value="${escapeAttr(dp.sourceAddr || '0x000000000001')}" placeholder="0x...">
                        </div>
                        <div>
                            <label class="block text-xs text-jd-textSecondary mb-1">目标地址 <span class="text-red-400">*</span></label>
                            <input type="text" class="jd-input w-full px-3 py-2 rounded-lg text-sm bg-white font-mono" id="instDestAddrInput" value="${escapeAttr(dp.destAddr || '0x000000000002')}" placeholder="0x...">
                        </div>
                        <div>
        ...
        area.innerHTML = instancePreview.map((step, index) => `
            <div class="mt-3 rounded-xl border border-jd-cardBorder bg-white p-3 space-y-2">
                ...
                <div>
                    ...
                    <div class="raw-hex-box hex-display text-[11px]">${AduCommon.formatHex(step.packetHex)}</div>
                </div>
            </div>
        `).join('');
    }
